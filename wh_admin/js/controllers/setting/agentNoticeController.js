'use strict';
app.controller('agentNoticeController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$modal', 'request', 'dialog',
    function ($scope, $http, $state, $stateParams, $timeout, $modal, request, dialog) {
        ...
        $scope.getNoticeList = function () {
            request.request('/gameop/notice', {uid: $scope.user.uid, type: 1})
        ...
        $scope.openNoticeModal = function () {
            var modalInstance = $modal.open({
                templateUrl: 'agentNoticeModal.html',
                controller: 'agentNoticeModalController',
                resolve: {
                    dataType: function () {
                        return '游戏通知';
                    }
            ...
            modalInstance.result.then(function (data) {
                ...
                if ($scope.text === null || $scope.text === undefined) {
                    ...
                    request.request('/user/sendNotice', {
                        uid: $scope.user.uid, title: '',
                        contents: $scope.text, type: 1
                    })
