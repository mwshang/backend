app.controller('gameUrgentNoticeController', ['$scope', '$http', '$state', '$stateParams', '$timeout', '$modal', 'request', 'dialog',
    function ($scope, $http, $state, $stateParams, $timeout, $modal, request, dialog) {
        ...
        $scope.getNoticeList = function () {
            request.request('/gameop/notice', {uid: $scope.user.uid, type: 10})
        ...
        $scope.openNoticeModal = function () {
            var modalInstance = $modal.open({
                templateUrl: 'gameUrgentNoticeModal.html',
                controller: 'gameUrgentNoticeModalController',
                resolve: {
                    dataType: function () {
                        return '游戏通知';
                    }
            ...
            modalInstance.result.then(function (data) {
                ...
                if ($scope.times === null || $scope.times === undefined || $scope.times < 0 || $scope.time > 50) {
